...
const DefenseArea: React.FC<{ title: string; content: string; icon: React.ReactNode; onChat: () => void }> = ({ title, content, icon, onChat }) => (
  <div className="group min-h-[60vh] flex flex-col justify-center py-20 border-b border-white/5 last:border-0 relative">
    <div className="flex flex-col md:flex-row gap-16 items-start">
      <div className="w-full md:w-1/3 space-y-6">
        <div className="p-5 bg-emerald-500/10 rounded-2xl w-fit text-emerald-400 group-hover:bg-emerald-500 group-hover:text-white transition-all duration-500">
          {icon}
        </div>
        <h3 className="text-4xl md:text-5xl font-black font-orbitron text-white transition-all duration-700 group-hover:text-emerald-400">{title}</h3>
      </div>
      <div className="w-full md:w-2/3">
        <p className="text-2xl md:text-3xl font-light font-space leading-[1.6] text-slate-400 transition-all duration-1000 group-hover:text-emerald-100">
          {content}
        </p>
      </div>
    </div>
...
const DefenseSection: React.FC<{ onOpenChat: () => void; onNavigate: (tab: Tab) => void }> = ({ onOpenChat, onNavigate }) => (
  <div className="min-h-screen py-32 px-12 md:px-24 flex flex-col max-w-7xl mx-auto relative">
    <BackgroundSymbols />
    <h2 className="text-5xl font-bold font-orbitron text-emerald-400 mb-20">Savunma Sistemleri</h2>
    <div className="space-y-10">
      <DefenseArea title="Harp Teknolojileri" content="İHA/SİHA ve otonom kara platformları için yüksek hassasiyetli seyrüsefer sistemleri." icon={<TargetIcon className="w-10 h-10" />} onChat={onOpenChat} />
      <DefenseArea title="Siber Güvenlik" content="Gelişmiş exploit koruma kalkanları ve yapay zeka destekli SIEM çözümleri." icon={<Lock className="w-10 h-10" />} onChat={onOpenChat} />
      <DefenseArea title="Teçhizat Üretimi" content="Balistik çelik yelekler, kompozit kasklar ve modern piyade silah sistemleri." icon={<Package className="w-10 h-10" />} onChat={onOpenChat} />
    </div>
  </div>
);
